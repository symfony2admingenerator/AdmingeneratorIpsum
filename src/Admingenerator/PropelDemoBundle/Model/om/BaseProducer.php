...
/**
 * Base class that represents a row from the 'propel_producers' table.
 *
 * 
 *
...
abstract class BaseProducer extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'Admingenerator\\PropelDemoBundle\\Model\\ProducerPeer';
	 ...
	 * @return     Producer The current object (for fluent API support)
	 */
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = ProducerPeer::NAME;
		}
	...
	public function setIsPublished($v)
	{
		...
		if ($this->is_published !== $v) {
			$this->is_published = $v;
			$this->modifiedColumns[] = ProducerPeer::IS_PUBLISHED;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = ProducerPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ProducerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ProducerQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ProducerPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ProducerPeer::ID) ) {
			...
			if ($this->collMovies !== null) {
				foreach ($this->collMovies as $referrerFK) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->collMovies) {
				$result['Movies'] = $this->collMovies->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
			}
		}
		return $result;
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProducerPeer::NAME)) $criteria->add(ProducerPeer::NAME, $this->name);
		if ($this->isColumnModified(ProducerPeer::IS_PUBLISHED)) $criteria->add(ProducerPeer::IS_PUBLISHED, $this->is_published);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getMovies() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMovie($relObj->copy($deepCopy));
				}
			}
	...
	{
		if ('Movie' == $relationName) {
			return $this->initMovies();
		}
	}
	 ...
	 *
	 * By default this just sets the collMovies collection to an empty array (like clearcollMovies());
	 ...
	 * @return     void
	 */
	public function initMovies($overrideExisting = true)
	{
		if (null !== $this->collMovies && !$overrideExisting) {
			return;
		}
	...
	/**
	 * Gets an array of Movie objects which contain a foreign key that references this object.
	 *
	...
	/**
	 * Returns the number of related Movie objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Movie objects.
	 * @throws     PropelException
	 */
	public function countMovies(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collMovies || null !== $criteria) {
			if ($this->isNew() && null === $this->collMovies) {
				return 0;
	...
	/**
	 * Method called to associate a Movie object to this object
	 * through the Movie foreign key attribute.
	 *
	 * @param      Movie $l Movie
	 * @return     Producer The current object (for fluent API support)
	 */
	public function addMovie(Movie $l)
	{
		if ($this->collMovies === null) {
			$this->initMovies();
		}
		if (!$this->collMovies->contains($l)) { // only add it if the **same** object is not already associated
			$this->collMovies[]= $l;
			$l->setProducer($this);
		}
	 ...
	 * been saved, it will retrieve related Movies from storage.
	 *
	 ...
	 * @return     PropelCollection|array Movie[] List of Movie objects
	 */
	public function getMoviesJoinActorHasMovieRelatedById($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = MovieQuery::create(null, $criteria);
		$query->joinWith('ActorHasMovieRelatedById', $join_behavior);

		return $this->getMovies($query, $con);
	}
